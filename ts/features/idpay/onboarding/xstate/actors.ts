...
const createActorsImplementation = (
  ...
  const loadInitiative = fromPromise(
    async ({ input }: { input: IdPayOnboardingMachineContext }) => {
  ...
  const loadInitiativeStatus = fromPromise(
    async ({ input }: { input: IdPayOnboardingMachineContext }) => {
  ...
  const loadRequiredCriteria = fromPromise(
    async ({ input }: { input: { initiative: InitiativeDataDTO } }) => {
      if (input.initiative === undefined) {
        return Promise.reject(OnboardingFailureEnum.GENERIC);
      }
  ...
  const acceptRequiredCriteria = fromPromise(
    async ({ input }: { input: IdPayOnboardingMachineContext }) => {
      const { initiative, requiredCriteria, multiConsentsAnswers } = input;
      ...
      const consentsArray = [
        ...getBoolRequiredCriteriaFromContext(input).map(_ => ({
          _type: _._type,
          code: _.code,
          accepted: true
        })),
...
export type IdPayOnboardingMachineActors = ReturnType<
  typeof createActorsImplementation
>;
